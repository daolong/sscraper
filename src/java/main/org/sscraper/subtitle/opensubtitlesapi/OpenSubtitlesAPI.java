...
package org.sscraper.subtitle.opensubtitlesapi;
...
public class OpenSubtitlesAPI {
	...
	public String login(String username, String password, String langauge, String useragent)
	...
	public List<Map<String, Object>> search(String token, String title, LANGUAGE language) throws OpenSubtitlesException {
		LANGUAGE[] languages = { language };
		return search(token, title, languages);
	}
	
	public List<Map<String, Object>> search(String token, File movie, LANGUAGE[] languages)
			throws OpenSubtitlesException {
		...
		try {
			...
			String hash = OpenSubtitlesHasher.computeHash(movie);
			Log.d("OpenSubtitlesApi", "search hash = " + hash);
	...
	public List<Map<String, Object>> search(String token, String title, LANGUAGE[] languages)
			throws OpenSubtitlesException {
		List<Map<String, Object>> results = new ArrayList<Map<String, Object>>();
		try {
			List<Object> params = new ArrayList<Object>();
			...
			if (resObject instanceof Object[]) {
				...
				Boolean result = (Boolean) resObject;
				if (!result) {
					log.info("No results found for " + title);
				}
			} else {
				throw new Exception("Not know object type: " + resObject.getClass());
			}
	...
	private enum API {
		...
		LOGIN("LogIn"), SEARCH("SearchSubtitles"), DOWNLOAD("DownloadSubtitles"), LOGOUT("LogOut");
